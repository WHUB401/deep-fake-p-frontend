...
<script>
import { deleteTraceResult, getTraceResultPage } from '@/api/trace'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getTraceResultPage(this.listQuery).then(response => {
    ...
    getDetectionResult(filename) {
      this.$router.push({ name: 'Trace', query: { filename: filename }})
    },
