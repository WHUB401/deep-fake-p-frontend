...
<script>
import { getForgeryResultPage, deleteForgeryResult } from '@/api/forgery'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getForgeryResultPage(this.listQuery).then(response => {
    ...
    getDetectionResult(filename) {
      this.$router.push({ name: 'ForgeryDetection', query: { filename: filename }})
    },
